...
将课前资料中的文件上传到linux下的`/home/hz/fdfs`目录:

 ![1561876520386](assets\1561876520386.png)
...
yum安装异常参考:<https://www.linuxidc.com/Linux/2014-07/104461.htm>
...
1）安装完成，我们应该能在`/etc/init.d/`目录，通过命令`ll /etc/init.d/ | grep fdfs`看到FastDFS提供的启动脚本：
 ...
 ![1524237469238](assets\1561881166950.png)
...
```shell
base_path=/hz/fdfs/tracker # tracker的数据和日志存放目录
...
```shell
base_path=/hz/fdfs/storage # storage的数据和日志存放目录
store_path0=/hz/fdfs/storage # storage的上传文件存放路径
...
- 配置config文件
  ...
  ```shell
  # 进入配置目录
  cd /home/hz/fdfs/fastdfs-nginx-module/src/
  # 修改配置
  vim config
...
- 修改一下配置：
  ...
  store_path0=/hz/fdfs/storage        		# 文件存储路径
  ```

- 复制 FastDFS的部分配置文件到/etc/fdfs目录

  ```shell
  cd /home/hz/fdfs/FastDFS/conf/
...
- 配置
  ...
  ```shell
  sudo ./configure --prefix=/opt/nginx --sbin-path=/usr/bin/nginx --add-module=/home/hz/fdfs/fastdfs-nginx-module/src
  ```
...
![1561885406863](assets\1561885406863.png)
