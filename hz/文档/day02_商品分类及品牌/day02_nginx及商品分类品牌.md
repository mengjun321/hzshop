...
```
127.0.0.1 api.hz.com
127.0.0.1 manage.hz.com
```
...
![1561658297554](assets\1561658297554.png) 

直接解压使用，效果：

![1561658412694](assets\1561658412694.png)

我们添加了两个映射关系：

- 127.0.0.1 api.hz.com ：我们的网关Zuul
- 127.0.0.1 manage.hz.com：我们的后台系统地址
...
![1561658257692](assets\1561658257692.png)
...
- 将制定文件通过xftp上传到 linux服务器 `/home/hz`

![1561648875261](assets\1.png)
...
这里指的是安装的目录在：/opt/nginx

![1561653263005](assets\1561653263005.png)
...
![1561649562065](assets\1561649562065.png)
...
- 防火墙开机关闭：`chkconfig iptables off`

![1561653365369](assets\1561653365369.png)

- 测试

![1561653410793](assets\1561653410793.png)
...
http {
	...
	server {
        ...
        location / {
			proxy_pass http://192.168.1.114:9001;
	...
	server {
        ...
        location / {
			proxy_pass http://192.168.1.114:10010;
			proxy_connect_timeout 600;
			proxy_read_timeout 600;
        }
    }
...
![1526189945180](assets/1561657929351.png)

1. 浏览器准备发起请求，访问http://mamage.hz.com，但需要进行域名解析
...
4. nginx中配置了反向代理规则，将manage.hz.com代理到127.0.0.1:9001，因此请求被转发
...
![1561690996518](assets\1561690996518.png)
...
- `v-card`：卡片，是vuetify中提供的组件，提供一个悬浮效果的面板，一般用来展示一组数据。
  ...
  ![1561692442135](assets\1561692442135.png)
...
http://manage.hz.com/item/category/list
...
![1526017217473](assets\1561692575048.png)

再次查看页面，发现地址已经变成了正确的地址了：

![1526017265625](assets\1561692699912.png)
...
需要注意的是，这里要用到mybatis-plus的注解，因此我们在`ly-item-iterface`中添加依赖
...
![1561692841583](assets\1561692841583.png))
...
![1527473096007](assets/1561692699912.png)
...
项目结构：
 ...
 ![1527483458604](assets\1561693195436.png)
...
我们不经过网关，直接访问：
 ...
 ![1561693306692](assets\1561693306692.png)

然后试试网关是否畅通：

  ![1561693349644](assets\1561693349644.png)
...
``` xml
/#/item/category:1 Access to XMLHttpRequest at 'http://api.hz.com/api/item/category/list?pid=0' from origin 'http://manage.hz.com' has been blocked by CORS policy: No 'Access-Control-Allow-Origin' header is present on the requested resource.
```

![1526017362418](assets\1561693656339.png)
...
而我们刚才是从`manage.hz.com`去访问`api.hz.com`，这属于二级域名不同，跨域了。
...
当浏览器发现发现的ajax请求是简单请求时，会在请求头中携带一个字段：`Origin`.
 ...
 ![1561693779537](assets\1561693779537.png)
...
```http
Access-Control-Allow-Origin: http://manage.hz.com
...
1.在需要跨域的Controller类中添加@CrossOrigin。需要为每一个Controller添加

``` java
@CrossOrigin  //解决跨域问题  自动
public class CategoryController {
...
访问正常：
 ...
 ![1561694388684](assets\1561694388684.png)

页面也OK了：

![1561694411816](assets\1561694411816.png)
...
![1561702310974](assets\1561702310974.png)
...
![1561702269884](assets\1561702269884.png)
...
![1561702215512](assets\1561702215512.png)
...
前台页面已经准备好，接下来就是后台提供数据接口了。
 ...
 http://api.hz.com/api/item/brand/page?key=kkk&page=1&rows=5&sortBy=id&desc=false
...
通过浏览器访问试试：http://api.hz.com/api/item/brand/page
...
// config中定义的基础路径是：http://api.hz.com/api
...
- http.js对axios进行了全局配置：`baseURL=config.api`，即`http://api.hz.com/api`。因此以后所有用axios发起的请求，都会以这个地址作为前缀。
